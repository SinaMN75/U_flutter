...
class ContentService {
  ...
  Future<(UResponse<List<UContentResponse>>?, UEmptyResponse?, String?)> read({
    ...
    await UHttpClient.send(
      ...
      body: p.toMap().add("apiKey", U.apiKey).add("token", ULocalStorage.getToken()),
      onSuccess: (Response r) {
        final UResponse<List<UContentResponse>> ok = UResponse<List<UContentResponse>>.fromJson(
          r.body,
          (dynamic i) => List<UContentResponse>.from((i as List<dynamic>).map((dynamic x) => UContentResponse.fromMap(x))),
        );
        result = (ok, null, null);
        onOk?.call(ok);
      },
  ...
  Future<(UResponse<UContentResponse>?, UEmptyResponse?, String?)> readById({
    ...
    (UResponse<UContentResponse>?, UEmptyResponse?, String?) result = (null, null, null);
    await UHttpClient.send(
      method: "POST",
      endpoint: "${U.baseUrl}/content/ReadById",
      body: p.toMap().add("apiKey", U.apiKey).add("token", ULocalStorage.getToken()),
      onSuccess: (Response r) {
        final UResponse<UContentResponse> ok = UResponse<UContentResponse>.fromJson(r.body, (dynamic i) => UContentResponse.fromMap(i));
        result = (ok, null, null);
        onOk?.call(ok);
      },
      onError: (Response r) {
  ...
  Future<(UEmptyResponse?, UEmptyResponse?, String?)> deleteRange({
    ...
    await UHttpClient.send(
      method: "POST",
      endpoint: "${U.baseUrl}/content/DeleteRange",
      body: p.toMap().add("apiKey", U.apiKey).add("token", ULocalStorage.getToken()),
      onSuccess: (Response r) {
        final UEmptyResponse ok = UEmptyResponse.fromJson(r.body);
        result = (ok, null, null);
