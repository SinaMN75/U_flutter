...
class BarcodeAztec extends Barcode2D {
  ...
  const BarcodeAztec(this.minECCPercent,
  ...
  List<_State> _updateStateForPair(_State s, List<int> data, int index, int pairCode) {
    ...
    if (pairCode == 3 || pairCode == 4) {
      final digitState = stateNoBinary
          ...
          .
      latchAndAppend(_EncodingMode.mode_digit, 1);
...
abstract class Barcode {
  ...
  Iterable<BarcodeElement> make(String data, {
  ...
  Iterable<BarcodeElement> makeBytes(Uint8List data, {
    required double width,
    required double height,
    bool drawText = false,
    double? fontHeight,
  ...
  void verifyBytes(Uint8List data) {
    ...
    for (var code in data) {
      if (!chr.contains(code)) {
        throw BarcodeException(String.fromCharCode(code)'Unable to encode "${}" to $name Barcode');
      }
    }
  }

  String toSvg(String data, {
    double x = 0,
    double y = 0,
    double width = 200,
    double height = 80,
  ...
  String toSvgBytes(Uint8List data, {
    double x = 0,
    double y = 0,
    double width = 200,
    double height = 80,
  ...
  String _toSvg(Iterable<BarcodeElement> recipe,
      double x,
      double y,
      double width,
      double height,
    ...
    for (var elem in recipe) {
      if (elem is BarcodeBar) {
        if (elem.black) {
          path.write(_d(x + elem.left)_d(y + elem.top)
          "M ${} ${} ");
    path.write(_d(elem.width)"h ${} ");
    path.write(_d(elem.height)"v ${} ");
    path.write(_d(-elem.width)"h ${} ");
    path.write("z ");
    }
    ...
    tSpan.write(_d(lX)_d(lY)_s(elem.text)'<tspan style="text-anchor: $anchor" x="${}" y="${}">${}</tspan>');
    }
    }

    final output = StringBuffer();
    if (fullSvg) {
    output.write(_d(x)_d(y)_d(width)_d(height)'<svg viewBox="${} ${} ${} ${}" xmlns="http://www.w3.org/2000/svg">');
    }

    output.write(_c(color)'<path d="$path" style="fill: ${}"/>');
    output.write(_c(color)_s(fontFamily)_d(fontHeight)_d(x)_d(y)'<text style="fill: ${}; font-family: &quot;${}&quot;; font-size: ${}px" x="${}" y="${}">$tSpan</text>');
...
abstract class Barcode1D extends Barcode {
  ...
  double marginTop(bool drawText,
      double width,
      double height,
      double fontHeight,
      double textPadding,) => 0;

  double marginLeft(bool drawText,
      double width,
      double height,
      double fontHeight,
      double textPadding,) => 0;

  double marginRight(bool drawText,
      double width,
      double height,
      double fontHeight,
      double textPadding,) => 0;

  Iterable<BarcodeElement> makeText(String data,
      double width,
      double height,
      double fontHeight,
      double textPadding,
      double lineWidth,) sync* {
    yield BarcodeText(
      left: 0,
...
class Barcode2DMatrix {
  ...
  factory Barcode2DMatrix.fromXY(int width,
      int height,
      double ratio,
...
class BarcodeBar extends BarcodeElement {
  ...
  @override
  String toString() => black ? "X" : " ""$runtimeType [${}] $left $top $width $height";
}
...
class BarcodeCodabar extends Barcode1D {
  ...
  const BarcodeCodabar(this.start,
      this.stop,
      this.printStartStop,
  ...
  void verifyBytes(Uint8List data) {
    if (explicitStartStop) {
      ...
      if (!validStartStop.contains(data[0])) {
        throw BarcodeException(String.fromCharCode(data[0])'Unable to encode $name Barcode: "${}" is an invalid start char');
      }

      if (!validStartStop.contains(data[data.length - 1])) {
        throw BarcodeException(String.fromCharCode(data[data.length - 1])'Unable to encode $name Barcode: "${}" is an invalid start char');
      }
...
class BarcodeCode128 extends Barcode1D {
  ...
  Iterable<int> shortestCode(List<int> data) {
    ...
    void addFrom(List<int> data, int start) {
      ...
      if (t == null) {
        throw BarcodeException(String.fromCharCodes(data)'Unable to encode "${}" to $name Barcode');
      }
...
class BarcodeDataMatrix extends Barcode2D {
  ...
  Iterable<BarcodeElement> make(String data, {
    ...
    final encoder = DataMatrixEncoder()
      ..ascii(data);
    return makeBytes(
      encoder.toBytes(),
      width: width,
...
class BarcodeEan13 extends BarcodeEan {
  ...
  Iterable<bool> convert(String data) sync* {
    ...
    if (first == null) {
      throw BarcodeException(String.fromCharCode(data.codeUnits.first)'Unable to encode "${}" to $name Barcode');
    }
...
class BarcodeEan2 extends BarcodeEan {
  ...
  double marginTop(bool drawText,
      ...
      double height,
      double fontHeight,
      double textPadding,) => drawText ? fontHeight + textPadding : 0;
...
class BarcodeItf extends BarcodeEan {
  ...
  const BarcodeItf(this.addChecksum,
      this.zeroPrepend,
      this.drawBorder,
      this.borderWidth,
      this.quietWidth,
      this.fixedLength,) : assert(fixedLength == null || fixedLength % 2 == 0);
  ...
  Iterable<bool> convert(String data) sync* {
    ...
    for (var i = 0; i < cu.length / 2; i++) {
      ...
      if (tuple[0] == null || tuple[1] == null) {
        throw BarcodeException(String.fromCharCode(cu[i * 2])String.fromCharCode(cu[i * 2 + 1])
        'Unable to encode "${}${}" to $name Barcode');
    }
  ...
  Iterable<BarcodeElement> makeBytes(Uint8List data, {
    ...
    if (drawBorder) {
      ...
      yield BarcodeBar(left: 0,
          ...
          black: true);
      yield BarcodeBar(left: width - bw,
          top: bw,
          width: bw,
          height: height - hp - bw * 2,
          black: true);
...
class BarcodeItf14 extends BarcodeItf {
  ...
  const BarcodeItf14(bool drawBorder,
      double? borderWidth,
      double? quietWidth,) : super(true, true, drawBorder, borderWidth, quietWidth, 14);
...
class BarcodeItf16 extends BarcodeItf {
  ...
  const BarcodeItf16(bool drawBorder,
      double? borderWidth,
      double? quietWidth,) : super(true, true, drawBorder, borderWidth, quietWidth, 16);
...
final class QrPayload {
  ...
  factory QrPayload.fromString(String data) =>
      QrPayload()
        ..addString(data);
...
class BarcodePDF417 extends Barcode2D {
  ...
  Iterable<int> _encodeNumeric(List<int> digits) sync* {
    ...
    for (var i = 0; i < chunkCount; i++) {
      ...
      var chunkNum = BigInt.parse(String.fromCharCodes(chunk)"1${}", radix: 10);
...
class BarcodePostnet extends BarcodeHM {
  ...
  Iterable<BarcodeHMBar> convertHM(String data) sync* {
    ...
    for (final codeUnit in data.codeUnits) {
      ...
      if (code == null) {
        throw BarcodeException(String.fromCharCode(codeUnit)'Unable to encode "${}" to $name');
      }
...
final class QrCode {
  ...
  static int _calculateTypeNumberFromPayload(QrErrorCorrectLevel errorCorrectLevel,
      QrPayload payload,
      int minTypeNumber,) {
...
List<int> getDataCache(QrCode code) =>
    code._dataCache ??= _createData(
      code.typeNumber,
...
List<int> _createData(int typeNumber,
  ...
  for (var i = 0; i < dataList.length; i++) {
    ...
    buffer..put(data.mode.value, 4)..put(data.length, data.mode.getLengthBits(typeNumber));
    data.write(buffer);
  }
...
final class QrImage {
  ...
  factory QrImage(QrCode qrCode) {
    ...
    final dataMap = Uint8List(dataSize)
      ..setRange(0, dataSize, template._data);
...
class BarcodeQR extends Barcode2D {
  ...
  const BarcodeQR(this.typeNumber,
      this.errorCorrectLevel,) : assert(typeNumber == null || (typeNumber >= 1 && typeNumber <= 40));
...
final class QrRsBlock {
  ...
  static List<QrRsBlock> getRSBlocks(int typeNumber,
      QrErrorCorrectLevel errorCorrectLevel,) {
  ...
  static int getTotalDataBits(int typeNumber,
      QrErrorCorrectLevel errorCorrectLevel,) {
    final rsBlock = _getRsBlockTable(typeNumber, errorCorrectLevel);
    final length = rsBlock.length ~/ 3;
    var totalDataBits = 0;
...
List<int> _getRsBlockTable(int typeNumber,
    QrErrorCorrectLevel errorCorrectLevel,) =>
...
BarcodeQRCorrectionLevel _mapEcc(UErrorCorrectionLevel level) =>
    switch (level) {
...
Barcode _barcodeFor(UBarcodeType type, {int? qrVersion, UErrorCorrectionLevel ecc = UErrorCorrectionLevel.low}) =>
    switch (type) {
...
class _UBarcodePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    if (background != null) canvas.drawRect(Offset.zero & size, Paint()
      ..color = background!);
    ...
    final Paint barPaint = Paint()
      ..isAntiAlias = moduleShape != UBarcodeModuleShape.square;
    if (gradient != null) {
  ...
  void _paintLogo(Canvas canvas, Rect area) {
    ...
    canvas.drawRRect(RRect.fromRectXY(padded, target * 0.15, target * 0.15), Paint()
      ..color = background ?? const Color(0xFFFFFFFF));
...
class UBarcodeExamples extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    Widget tile(String title, Widget child) =>
        Padding(
