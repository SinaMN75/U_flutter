...
class _UAdminGoldPageState extends State<UAdminGoldPage> {
    ...
    body: UDefaultTabBar(
      ...
      tabBar: TabBar(
        ...
        tabs: <Widget>[
          ...
          Tab(text: U.s.goldProviderTransactions),
          Tab(text: U.s.goldTradeLimits),
  ...
  Widget _overview() => Obx(
    () => SingleChildScrollView(
      ...
      child: UColumn(
        ...
        spacing: 12,
        children: <Widget>[
          _card(U.s.goldProviderAccount, <UAdminField>[
            UAdminField(U.s.title, c.account.value?.name ?? "-"),
            UAdminField(U.s.goldAccountStatus, (c.account.value?.active ?? false) ? U.s.active : U.s.inactive),
            UAdminField(U.s.goldIpWhitelist, (c.account.value?.ipWhitelist ?? <String>[]).join(", ")),
          ]),
          _card(U.s.goldPrice, <UAdminField>[
            UAdminField(U.s.goldBuyPrice, c.quote.value?.buyUnitPrice.rial()),
            UAdminField(U.s.goldSellPrice, c.quote.value?.sellUnitPrice.rial()),
            UAdminField(U.s.goldPricePerGram, c.quote.value?.baseUnitPrice.rial()),
            UAdminField(U.s.goldLastUpdated, c.quote.value?.updatedAt?.toJalaliDateTime() ?? "-"),
          ]),
          _card(
            U.s.goldProviderBalances,
            c.balances
  ...
  Widget _orders() => UColumn(
    children: <Widget>[
      UAdminListView<UGoldOrderResponse>(
        ...
        emptyText: U.s.goldNoOrdersYet,
        desktopHeader: () => UAdminTable.header(<String>[U.s.type, U.s.goldAmountInGram, U.s.amount, U.s.goldUnitPrice, U.s.status, U.s.date]),
      ...
      Obx(
        () => c.ordersCursor == null
            ...
            : UButton(title: U.s.goldLoadMore, type: UButtonType.text, onTap: () => c.readOrders(more: true)).pOnly(bottom: 8),
      ),
    ],
  );
  ...
  Widget _orderMobile(UGoldOrderResponse i, int index) => UAdminTable.mobileCard(
    ...
    title: "${i.side?.localizedTitle ?? "-"} • ${i.dealtBaseAmount?.toStringAsSmartRound(maxPrecision: 4) ?? "-"} ${U.s.goldGram}",
    ...
    fields: <UAdminField>[
      ...
      UAdminField(U.s.goldUnitPrice, i.effectivePrice.rial()),
      UAdminField(U.s.goldOrderId, i.id),
  ...
  Widget _transactions() => UColumn(
    children: <Widget>[
      UAdminListView<UGoldTransactionResponse>(
        ...
        emptyText: U.s.goldNoTransactionsYet,
      ...
      Obx(
        () => c.txnsCursor == null
            ? const SizedBox.shrink()
            : UButton(title: U.s.goldLoadMore, type: UButtonType.text, onTap: () => c.readTransactions(more: true)).pOnly(bottom: 8),
      ),
    ],
  );
  ...
  Widget _limits() => Obx(
    () => SingleChildScrollView(
      ...
      child: UColumn(
        ...
        spacing: 12,
        children: <Widget>[
          _card(U.s.goldTradeLimits, <UAdminField>[
            UAdminField(U.s.goldTimezone, c.limits.value?.timezone ?? "-"),
          ...
          _card(U.s.goldCreditFacilities, <UAdminField>[
            UAdminField(U.s.goldTimezone, c.credit.value?.timezone ?? "-"),
            ...(c.credit.value?.items ?? <UGoldCreditFacilityResponse>[]).map(
              (UGoldCreditFacilityResponse f) => UAdminField("${f.type ?? "-"} (${f.asset ?? "-"})", "${U.s.goldAvailableCredit}: ${f.availableCredit.rial()}"),
            ),
  ...
  Widget _tokens() => UColumn(
    children: <Widget>[
      UButton(title: U.s.goldCreateApiToken, icon: const Icon(Icons.add), fullWidth: true, onTap: _showCreateTokenDialog).pAll(16),
      UAdminListView<UGoldApiTokenResponse>(
        state: c.tokensState,
        items: () => c.tokens,
        totalCount: () => c.tokens.length,
        onRetry: c.readTokens,
        emptyText: U.s.goldNoApiTokens,
        desktopHeader: () => UAdminTable.header(<String>[U.s.goldTokenLabel, U.s.goldTokenPrefix, U.s.goldScopes, U.s.status, U.s.operations]),
        ...
        mobileRow: (UGoldApiTokenResponse i, int index) => UAdminTable.mobileCard(
          ...
          fields: <UAdminField>[
            UAdminField(U.s.goldScopes, i.scopes.join(", ")),
            UAdminField(U.s.goldIpWhitelist, i.ipWhitelist.join(", ")),
            UAdminField(U.s.date, i.createdAt?.toJalaliDateTime() ?? "-"),
          ],
        ),
      ).expanded(),
  ...
  void _showCreateTokenDialog() => UNavigator.dialog(
    AlertDialog(
      title: Text(U.s.goldCreateApiToken),
      content: SizedBox(
        ...
        child: SingleChildScrollView(
          child: UColumn(
            ...
            children: <Widget>[
              UTextField(controller: c.tokenLabelController, labelText: U.s.goldTokenLabel).pSymmetric(vertical: 6),
              UTextField(controller: c.tokenScopesController, labelText: U.s.goldScopes).pSymmetric(vertical: 6),
              UTextField(controller: c.tokenIpsController, labelText: U.s.goldIpWhitelist).pSymmetric(vertical: 6),
